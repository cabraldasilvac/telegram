<?php
echo" bot conectado! ";
// $botToken = "488011316:AAGv2EbQdvYdb28A-EzIy5WhqZkjSgX6Y7";
...
//Nós extraímos o comando
$arr = explode(' ',trim($message));
$command = $arr[0];

$message = substr(strstr($message," "), 1);

//Eles não exigem variáveis de usuário.

switch ($command) {
    case '/start':
        $response = "Bem vindo ao nosso Bot teste.";
        sendMessage($chatId, $response);
        break;
	case '/ajuda':
		$response = "Fique tranquilo, estou aqui com você.";
		sendMessage($chatId, $response);
		break;
	case '/ajuda2':
		$response = "Fique tranquilo, estou aqui com você.";
		$keyboard = '["Muito Obrigado"],["Ok"]';
